...
export const benchPapers: BenchPaper[] = [
  ...
  },
  {
    ...
    targetVenue: "PRIMUS",
    abstract:
      "[DRAFT — pilot data section marked as forthcoming.] The Math 2BL curriculum at Foothill College lists a 'Neural Networks Towards Machine Learning' project that has been marked under development for several years. We present the first complete version of this project. Rather than treating neural networks as a separate paradigm bolted onto a linear algebra course, we frame a single-hidden-layer network as a sequence of matrix operations whose learned weight matrices have geometrically interpretable singular values. Students train a network on the USPS handwritten digit dataset, then perform an SVD on each weight matrix and visualize the top singular vectors as eigen-strokes that the network has learned to detect. We argue this inversion — train first, decompose second — gives community college students an honest, mechanistic account of what a neural network is doing without requiring multivariable calculus. The complete laboratory worksheet, Colab notebook, and eight-step modeling rubric are in hand; a small pilot study of student conceptual gains is planned for a Math 2BL section and will be reported in the full draft.",
    researchQuestions: [
      {
        id: "RQ1",
        q:
