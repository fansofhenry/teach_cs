...
export const benchPapers: BenchPaper[] = [
  {
    ...
    targetVenue: "PRIMUS",
    abstract:
      "Anderson (2018) introduced the Make Eigenvalues Resonate (MER) project as a hands-on bridge from introductory linear algebra to vibrations analysis using a spring-coupled pair of pendula. Six years later, the project still depends on ad-hoc hardware and bespoke video analysis, which limits adoption outside Anderson's own classroom. We present MER 2.0: a fully reproducible, sub-$80 hardware bill of materials, an open-source OpenCV tracking pipeline that runs from any phone-recorded video, and a three-mass extension that lets students see — and verify by measurement — the eigenstructure of a system with multiple natural frequencies. We report a quarter-long pilot at Foothill College in which N students built the kit, fit eigenvalue-predicted frequencies to physical data within X% error, and self-reported gains on Anderson's five learner-centered objectives. We argue that low-cost reproducible hardware is the binding constraint on the spread of modeling-rich linear algebra labs, and offer MER 2.0 as a template for hardening other projects in the Math 2BL family.",
    researchQuestions: [
      {
        id: "RQ1",
        q:
